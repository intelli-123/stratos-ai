...
export function VBars({ data }: { data: (DailyPoint & { tooltip?: string })[] }) {
  const max = Math.max(...data.map((d) => d.cost), 0.0000001);
  return (
    <div className="vbars">
      {data.map((d) => (
        <div key={d.date} className="vbar-col" data-tooltip={d.tooltip}>
...
export function HBars({ data }: { data: (AgentSpend & { tooltip?: string })[] }) {
  const max = Math.max(...data.map((d) => d.cost), 0.0000001);
  if (!data.length) return <div className="muted" style={{ padding: "8px 0" }}>No spend yet.</div>;
  return (
    <div className="hbars">
      {data.map((d) => (
        <div key={d.name} className="hbar-row" data-tooltip={d.tooltip}>
